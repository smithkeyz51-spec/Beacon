            ...
            <div className="device-list">
              ...
              {creating && (
                <form onSubmit={handleCreateDevice} className="panel" style={{ marginBottom: 'var(--space-3)' }}>
                  <div className="field" style={{ marginBottom: 'var(--space-3)' }}>
                    ...
                    <input id="deviceName" placeholder="e.g. Sam's iPhone" value={newName} onChange={(e) => setNewName(e.target.value)} autoFocus />
                  </div>
                  <button className="btn btn-primary" type="submit" style={{ width: '100%' }}>Create device</button>
                </form>
              )}
              ...
              {devices.length === 0 && !creating && (
                <div className="panel empty-state">
                  ...
                  <p style={{ margin: 0, fontSize: 'var(--text-sm)' }}>Add a device, then generate a link to open on that phone.</p>
                  <button className="btn btn-primary" onClick={() => setCreating(true)}>+ Add device</button>
                </div>
              )}

              {devices.map((d) => (
                <DeviceCard key={d.id} device={d} active={d.id === selectedId} onSelect={() => setSelectedId(d.id)} onGetLink={() => handleGetLink(d.id)} />
              ))}
              ...
              <div className="coord-strip">
                {selected?.lastLocation ? (
                  <>
                    ...
                    <span style={{ marginLeft: 'auto' }}><LastSeen ts={selected.lastSeen} /></span>
                  </>
                ) : (
...
function DeviceCard({ device, active, onSelect, onGetLink }) {
  ...
  return (
    <div className={`device-card${active ? ' active' : ''}`} onClick={onSelect}>
      ...
      <div className="device-meta"><LastSeen ts={device.lastSeen} prefix="Last seen " /></div>
      <div style={{ marginTop: 'var(--space-3)' }}>
        <button className="btn" style={{ fontSize: 'var(--text-xs)', padding: '6px 10px' }} onClick={(e) => { e.stopPropagation(); onGetLink() }}>
          Get tracking link
        </button>
      </div>
    </div>
