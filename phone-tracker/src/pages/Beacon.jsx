...
export default function Beacon() {
  ...
  const [status, setStatus] = useState('resolving') // resolving | denied | invalid | active | error
      ...
      watchIdRef.current = navigator.geolocation.watchPosition(
        async (pos) => {
          ...
          try {
            ...
            // Keep watching even if a single write fails (e.g. brief offline blip)
    ...
    <div className="beacon-page">
      ...
      {status === 'active' && (
        <>
          ...
          <p>
            Keep this page open (or leave it added to your home screen) to keep sharing
            this device's location. Closing it stops the updates.
          </p>
      ...
      {status === 'denied' && (
        <>
          ...
          <p>
            This page can't report location because permission was denied. Open your
            browser's site settings and allow location for this page, then reload.
          </p>
        </>
      )}
      ...
      {status === 'invalid' && (
        <>
          ...
          <p>
            This tracking link is invalid or has expired. Ask the device owner to
            generate a new one from their dashboard.
          </p>
        </>
      )}
